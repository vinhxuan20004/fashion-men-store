...
import { Save, Trash2, Upload, X, ChevronLeft, ShoppingBag, Tag, Layers, Star, Info, Image as ImageIcon, Sparkles, RefreshCw } from 'lucide-react'
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 pb-8 border-b border-slate-100">
        <div>
          ...
          <div className="flex items-center gap-3">
             ...
             <h1 className="text-2xl font-bold text-slate-900 uppercase">Chỉnh sửa sản phẩm</h1>
          </div>
        </div>
          ...
          <div className="bg-white rounded-[2.5rem] border border-slate-100 p-10 shadow-xl shadow-slate-100/50">
             ...
             <div className="space-y-8">
                ...
                <div className="space-y-6">
                   ...
                   <div className="flex items-center justify-between p-6 bg-slate-50 border border-slate-100 rounded-2xl group cursor-pointer hover:bg-emerald-600 transition-all duration-500">
                      <div className="flex items-center gap-3">
                         <Star className="w-5 h-5 text-emerald-400 group-hover:scale-125 transition-transform" />
                         <span className="text-[10px] font-black text-slate-900 uppercase tracking-widest group-hover:text-white">Hiển thị</span>
                      </div>
                      <input
                        type="checkbox"
                        {...register('isActive')}
             ...
             <div className="grid grid-cols-2 gap-4">
                ...
                {existingImages.map((imgUrl, idx) => (
                  <div key={`existing-${idx}`} className="relative aspect-[3/4] group">
                    <img src={getImageUrl(imgUrl)} alt="" className="w-full h-full object-cover rounded-2xl shadow-md border border-slate-100" />
                    <button
                      type="button"
                    ...
                    {idx === 0 && (
                      <div className="absolute bottom-2 left-2 px-3 py-1 bg-slate-900/80 backdrop-blur-md text-[8px] font-black text-white uppercase tracking-widest rounded-lg">EXISTING</div>
                    )}
                  </div>
                ))}
                ...
                {imagePreviews.map((preview, idx) => (
                  <div key={`new-${idx}`} className="relative aspect-[3/4] group">
                    <img src={preview} alt="" className="w-full h-full object-cover rounded-2xl shadow-md border border-emerald-100 ring-2 ring-emerald-50" />
                    <button
                      type="button"
                    ...
                    <div className="absolute bottom-2 left-2 px-3 py-1 bg-emerald-600/80 backdrop-blur-md text-[8px] font-black text-white uppercase tracking-widest rounded-lg">NEW</div>
                  </div>
                ))}
                ...
                <label className="aspect-[3/4] border-2 border-dashed border-slate-100 rounded-2xl flex flex-col items-center justify-center cursor-pointer hover:border-primary-400 hover:bg-primary-50 transition-all group">
                   ...
                   <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest">TẢI THÊM ẢNH</span>
                   <input
                     type="file"
                     multiple
                     accept="image/*"
