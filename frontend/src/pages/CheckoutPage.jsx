...
import { CreditCard, Truck, Building2, Smartphone, ChevronRight, Lock, MapPin, ShieldCheck, ArrowLeft } from 'lucide-react'
import { useCart } from '../contexts/CartContext'
import { useAuth } from '../contexts/AuthContext'
import { orderAPI, paymentAPI, siteAPI } from '../services/api'
  ...
  React.useEffect(() => {
    const fetchMethods = async () => {
      try {
        ...
        const status = settings.payment_methods_status || { cod: true, bankTransfer: true, vnpay: true, momo: true }
        const bankData = settings.payment_bank_transfer
        ...
        const allMethods = [
          ...PAYMENT_METHODS,
          {
            ...
            description: bankData 
              ? `Chuyển khoản vào ${bankData.bankName} - ${bankData.accountNo}`
              : 'Thanh toán bằng cách chuyển khoản ngân hàng thủ công.',
            icon: Building2,
        ...
        const filtered = allMethods.filter(m => {
          if (m.id === 'COD') return status.cod
          if (m.id === 'BANK_TRANSFER') return status.bankTransfer
          return true
        })
  ...
  const onSubmit = async (formData) => {
    ...
    try {
      ...
      if (paymentMethod === 'COD' || paymentMethod === 'BANK_TRANSFER') {
        await clearCart()
        toast.success(paymentMethod === 'COD' ? 'Đặt hàng thành công!' : 'Đã tạo đơn hàng. Vui lòng chuyển khoản để hoàn tất.')
              ...
              <div className="bg-slate-50 rounded-[3rem] p-10 border border-slate-100 sticky top-28 backdrop-blur-3xl shadow-2xl shadow-slate-200/50">
                 ...
                 <div className="space-y-4 mb-10 pb-8 border-b border-slate-200/50">
                    <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-400">
                      <span>Tạm tính</span>
                      <span className="text-slate-900 price-bold">{formatCurrency(cartSubtotal)}</span>
                    </div>
                    {discount > 0 && (
                    ...
                    <div className="flex justify-between items-center text-[10px] font-black uppercase tracking-widest text-slate-400">
                      ...
                      <span className={`price-bold ${shipping === 0 ? 'text-emerald-600' : 'text-slate-900'}`}>
                 ...
                 <div className="flex justify-between items-center mb-10">
                    ...
                    <span className="text-3xl font-display font-black text-primary-600 tracking-tighter italic price-bold">
