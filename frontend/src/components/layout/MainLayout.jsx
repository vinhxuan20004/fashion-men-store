...
const MainLayout = ({ children }) => {
  ...
  useEffect(() => {
    ...
    return () => document.body.classList.remove('is-auth-page')
  }, [location.pathname])
  ...
  const navLinkClass = (path) =>
    `relative text-sm transition-all duration-300 hover:text-primary-600 ${
  ...
  return (
    <div className={`min-h-screen flex flex-col font-sans ${isAuthPage ? 'is-auth-page' : ''}`}>
      {/* Header */}
      <header 
                ...
                {catMenuOpen && (
                  <div className="absolute top-full left-0 mt-3 w-56 bg-white rounded-2xl shadow-2xl border border-slate-100 py-3 z-50 animate-slide-up">
                    <div className="px-4 py-2 mb-2 border-b border-slate-50">
                      <span className="text-[10px] text-slate-400 tracking-wide">Bộ sưu tập</span>
                    </div>
                    {Array.isArray(categories) && categories.map((cat) => (
                      <Link
                        ...
                        className="flex items-center justify-between px-4 py-2.5 text-sm text-slate-600 hover:bg-primary-50 hover:text-primary-700 transition-all rounded-lg mx-2 title-case"
                      >
                    ...
                    <div className="absolute top-full right-0 mt-3 w-60 bg-white rounded-2xl shadow-2xl border border-slate-100 py-2 z-50 animate-slide-up">
                      ...
                      {isAdmin && (
                          <Link
                            to="/admin"
                            onClick={() => setUserMenuOpen(false)}
                            className="flex items-center gap-3 px-4 py-2.5 text-sm text-primary-600 hover:bg-primary-50 transition-all rounded-lg mx-2"
                          >
                      ...
                      <div className="border-t border-slate-50 mt-2 pt-2">
                          <button
                            ...
                            className="flex items-center gap-3 w-full px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 transition-all rounded-lg mx-2"
                          >
        ...
        <div className="lg:hidden fixed inset-0 top-0 bg-white z-[100] animate-fade-in overflow-y-auto">
          ...
          <div className="p-6">
            ...
            <div className="space-y-6">
              <div>
                <h3 className="text-[10px] text-slate-400 tracking-wide mb-4">Điều hướng</h3>
                <nav className="flex flex-col gap-4">
                  <Link to="/" className="text-xl text-slate-900">Trang chủ</Link>
                  <Link to="/products" className="text-xl text-slate-900">Tất cả sản phẩm</Link>
                </nav>
              </div>

              <div>
                <h3 className="text-[10px] text-slate-400 tracking-wide mb-4">Danh mục</h3>
                <nav className="flex flex-col gap-3">
                  {Array.isArray(categories) && categories.map((cat) => (
                    <Link
                      key={cat._id}
                      to={`/products?category=${cat._id}`}
                      className="text-base text-slate-600 hover:text-primary-600 title-case"
                    >
                      {cat.name?.toLowerCase()}
                    </Link>
                  ))}
              ...
              {!isAuthenticated ? (
                <div className="flex flex-col gap-3 pt-6 border-t border-slate-100">
                  <Link to="/login" className="w-full text-center py-3 border border-slate-200 rounded-lg font-bold text-slate-700 text-sm tracking-tight shadow-sm">
                    Đăng nhập
                  </Link>
                  <Link to="/register" className="w-full text-center py-3 bg-primary-600 text-white rounded-lg font-bold text-sm tracking-tight shadow-sm">
                    Tạo tài khoản
                  </Link>
                </div>
              ) : (
                <div className="pt-6 border-t border-slate-100 space-y-4">
                  <Link to="/profile" className="flex items-center gap-3 text-lg text-slate-700">
                    <User className="w-5 h-5 text-primary-600" />
                    Hồ sơ của tôi
                  </Link>
                  <Link to="/orders" className="flex items-center gap-3 text-lg text-slate-700">
                    <Package className="w-5 h-5 text-primary-600" />
                    Đơn hàng
                  </Link>
                  <Link to="/settings" className="flex items-center gap-3 text-lg text-slate-700">
                    <Settings className="w-5 h-5 text-primary-600" />
                    Cài đặt
                  </Link>
                  {isAdmin && (
                    <Link to="/admin" className="flex items-center gap-3 text-lg text-primary-600">
            ...
            <div>
              ...
              <ul className="space-y-5 text-sm">
                ...
                <li className="flex gap-4">
                  ...
                  <a href="tel:+84901234567" className="text-slate-400 hover:text-white transition-colors">0901 234 567 <br/><span className="text-[10px] text-slate-500 tracking-wide">(Hotline 24/7)</span></a>
                </li>
                <li className="flex gap-4">
                  <div className="w-10 h-10 rounded-xl bg-white/5 flex flex-shrink-0 items-center justify-center">
                ...
                <li className="flex gap-4">
                  ...
                  <span className="text-slate-400">08:00 - 21:00 <br/><span className="text-[10px] text-slate-500 tracking-wide">Thứ 2 - Chủ nhật</span></span>
                </li>
              </ul>
            </div>
          </div>
