...
const COLORS = [
  ...
  { name: 'Xám', value: 'gray', code: '#9CA3AF' },
...
const FilterSection = ({ title, children, defaultOpen = true }) => {
  ...
  return (
    <div className="border-b border-slate-50 py-6 last:border-0">
      ...
      >
        <h4 className="text-[10px] font-black text-slate-900 uppercase tracking-[0.2em] group-hover:text-primary-600 transition-colors">{title}</h4>
    ...
    <div className="bg-white">
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-slate-100">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-black text-slate-900 uppercase tracking-widest italic">Bộ lọc</h3>
        </div>
        <button
          onClick={onReset}
            ...
            {categories.map((cat) => {
              ...
              return (
                <label key={cat._id} className="flex items-center group cursor-pointer">
                  ...
                  <span className={`ml-3 text-xs font-bold transition-colors ${active ? 'text-slate-900' : 'text-slate-500 group-hover:text-slate-700'}`}>
                    {cat.name}
                  </span>
            ...
            {SIZES.map((size) => {
              ...
              return (
                <button
                  ...
                  className={`h-10 text-[10px] font-black rounded-lg border-2 transition-all duration-300 ${
                    active
        ...
        <FilterSection title="Khoảng giá">
          <div className="space-y-4">
            <div className="grid grid-cols-2 gap-2">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full bg-slate-50 border border-slate-100 rounded-xl pl-9 pr-3 py-3 text-xs font-bold text-slate-900 focus:outline-none focus:ring-2 focus:ring-primary-100 transition-all placeholder:text-slate-300"
                />
              </div>
              <div className="relative">
                <span className="absolute left-3 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-300 uppercase">Đến</span>
                <input
                  type="number"
                  value={maxPrice}
            ...
            <button
              ...
              className="w-full bg-primary-600 text-white py-3 rounded-xl text-[10px] font-black uppercase tracking-widest shadow-lg shadow-primary-600/20 hover:bg-primary-700 active:scale-95 transition-all"
            >
              Áp dụng lọc giá
            </button>
            ...
            <div className="grid grid-cols-2 gap-2 mt-4">
              {[
                ...
                <button
                  ...
                  className="text-[10px] font-black uppercase tracking-tight py-2 rounded-lg bg-white border border-slate-100 text-slate-500 hover:border-primary-600 hover:text-primary-600 transition-all"
                >
                  {range.label}
                </button>
              ))}
