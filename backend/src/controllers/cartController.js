...
const addItem = async (req, res, next) => {
  try {
    ...
    if (!productId || !size || !color) {
      return res.status(400).json({ success: false, message: 'productId, size, and color are required.' });
    }
    ...
    if (!variant) {
      return res.status(404).json({ success: false, message: `Variant (size: ${size}, color: ${color}) not found.` });
    }
